...
import { saveMiscRevenueAction } from "./actions";
import {
  initialMiscRevenueFormState,
  type MiscRevenueFormState,
} from "./state";
...
export function MiscRevenueFormDialog({ mode, defaults }: Props) {
  const [open, setOpen] = useState(false);
  const [state, dispatch] = useActionState<MiscRevenueFormState, FormData>(
    saveMiscRevenueAction,
    initialMiscRevenueFormState,
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {mode === "create" ? (
          <button
            type="button"
            className="tap inline-flex h-10 items-center gap-1.5 rounded-lg bg-ok px-4 text-sm font-semibold text-white shadow-sm hover:-translate-y-px hover:shadow-lg"
          >
      ...
      <DialogContent className="flex max-h-[90vh] max-w-md flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="shrink-0 border-b border-line px-6 pb-4 pt-6">
          <DialogTitle className="font-display text-lg font-bold">
            {mode === "create" ? "Nova receita avulsa" : "Editar receita"}
          </DialogTitle>
          <DialogDescription className="text-xs text-subtle">
            Venda fora de comanda, dica, lançamento manual de receita.
          </DialogDescription>
        </DialogHeader>
        ...
        <form action={dispatch} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 space-y-3.5 overflow-y-auto px-6 py-4">
            ...
            <Field
              ...
              placeholder="Ex: Pomada vendida balcão"
              required
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field
                ...
                type="date"
                defaultValue={
                  defaults ? dateIso(defaults.occurredAt) : todayIso()
                }
                required
                mono
...
function SubmitButton() {
  ...
  return (
    <button
      ...
      disabled={pending}
      className={cn(
        "inline-flex h-10 items-center gap-1.5 rounded-lg bg-ok px-4 text-sm font-semibold text-white shadow-sm",
